...
# Executar como Administrador APÓS reinício do Setup-Server.ps1
...
Write-Host "`n-- Password de Recuperacao (DSRM) --" -ForegroundColor White
...
if ($isDC -ge 4) {
    ...
    Write-Host "        Dominio atual: $((Get-WmiObject Win32_ComputerSystem).Domain)" -ForegroundColor Gray
    exit 0
}

# ── 2. PROMOVER A DC ────────────────────────────────────────────
Write-Host "`n[2/3] A promover Domain Controller para '$DomainName'..." -ForegroundColor Yellow
...
# ── 3. LOG (só corre se NoRebootOnCompletion = $true) ───────────
Write-Host "`n[3/3] A guardar log..." -ForegroundColor Yellow
...
Write-Host "`n  Apos reinicio, verifica com:" -ForegroundColor White
Write-Host "  Get-ADDomain" -ForegroundColor Yellow
Write-Host "  Resolve-DnsName $DomainName" -ForegroundColor Yellow
Write-Host "`n  Depois junta o cliente ao dominio '$DomainName'" -ForegroundColor White
Write-Host "  e aponta o DNS do cliente para o IP do servidor." -ForegroundColor White
Write-Host "============================================`n" -ForegroundColor Cyan
